...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $stats = [
            ...
            'payments_completed' => (clone $paymentsQuery)->where('status', 'completed')->sum('amount'),
            'profile_completion' => $user->profile_completion,
            'last_order_date' => optional($recentOrders->first())->created_at,
        ];
        $addresses = $user->addresses()->get();

        return view('front.account.dashboard', compact('stats', 'recentOrders', 'recentPayments', 'addresses', 'user'));
    }
}
