...
class SimpleAIService
{
    ...
    private function getPrompt(string $title, string $type, ?string $locale = null): string
    {
        ...
        return match ($type) {
            ...
            'homepage_section' => "{$languageInstruction}Create homepage section content for '{$title}'. Return JSON with: title (section title, max 100 chars), subtitle (section subtitle, max 200 chars), cta_label (call-to-action button text, max 50 chars). Make it engaging and action-oriented for e-commerce.",
            'translation' => "{$languageInstruction}Translate the following text/key to {$language} language. Return JSON with: translation (the translated text, max 500 chars). Make it natural and contextually appropriate for the target language.",
