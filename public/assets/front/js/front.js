...
(function () {
    ...
    // Product variations helpers
    function updateAvailableOptions(variations, selectedAttrs) {
        $$('.variation-attr-block').forEach(block => {
            ...
            radios.forEach(radio => {
                ...
                const hasMatch = variations.some(v => {
                    const attrData = v.attribute_data || {};
                    if (v.active === false) return false;
                    return Object.keys(testAttrs).every(attr => attrData[attr] === testAttrs[attr]);
                });
    ...
    function updatePrice(variations, selectedAttrs, currencySymbol) {
        ...
        if (!selectedAttrs || Object.keys(selectedAttrs).length === 0) {
            if (priceElement && priceMaxElement && priceRangeSep) {
        ...
        if (matchingVariation) {
            ...
            if (priceElement) priceElement.textContent = currencySymbol + ' ' + displayPrice.toFixed(2);
            ...
            if (variationInput) variationInput.value = matchingVariation.id;
    ...
    function updateStockStatus(variation) {
        ...
        if (stockBadge) {
            stockBadge.classList.remove('high-stock', 'low-stock', 'out-stock', 'badge-info');
            if (!variation.manage_stock) {
                stockBadge.classList.add('badge-info');
                stockBadge.textContent = stockBadge.dataset.unlimited || 'Unlimited';
            } else if (availableStock <= 0) {
                stockBadge.classList.add('out-stock');
                stockBadge.textContent = stockBadge.dataset.outOfStock || 'Out of Stock';
            } else if (availableStock <= 5) {
                stockBadge.classList.add('low-stock');
                stockBadge.textContent = stockBadge.dataset.lowStock || 'Low Stock';
            } else {
                stockBadge.classList.add('high-stock');
                stockBadge.textContent = stockBadge.dataset.inStock || 'In Stock';
            }
        }
        if (stockStatus) {
            if (!variation.manage_stock) {
                stockStatus.textContent = stockStatus.dataset.inStock || 'In stock';
            } else if (availableStock <= 0) {
                stockStatus.textContent = stockStatus.dataset.outOfStock || 'Out of stock';
            } else {
                stockStatus.textContent = availableStock + ' ' + (stockStatus.dataset.inStockText || 'in stock');
            }
        }
        if (addToCartBtn) {
            if (!variation.manage_stock || availableStock > 0) {
                addToCartBtn.disabled = false;
                addToCartBtn.classList.remove('btn-out-of-stock');
                addToCartBtn.textContent = addToCartBtn.dataset.addText || 'ADD TO CART';
                if (qtyInput) {
                    qtyInput.max = variation.manage_stock ? availableStock : 999;
    ...
    function initProductVariations() {
        ...
        $$('.attr-radio').forEach(radio => radio.addEventListener('change', function () { const selectedAttrs = {}; $$('.attr-radio:checked').forEach(checked => { const attrName = checked.name.replace('attr_', ''); selectedAttrs[attrName] = checked.value; }); updateAvailableOptions(variations, selectedAttrs); updatePrice(variations, selectedAttrs, currencySymbol); }));
        updateAvailableOptions(variations, {});
    }
    ...
    // Shipping helpers
    async function loadGovernorates(countryId, governorateSelect, citySelect, shippingZoneSelect, translations, hideShippingInfo) {
        if (!countryId) {
            if (governorateSelect) clearSelect(governorateSelect, translations.selectGovernorate);
            if (citySelect) clearSelect(citySelect, translations.selectCity);
            if (shippingZoneSelect) clearSelect(shippingZoneSelect, translations.selectShippingCompany);
            hideShippingInfo();
            return;
        }
        try {
            const data = await fetchJson(`/api/locations/governorates?country=${countryId}`);
    ...
    async function loadCities(governorateId, citySelect, shippingZoneSelect, countrySelect, translations, hideShippingInfo, loadShippingOptions) {
        if (!governorateId) {
            if (citySelect) clearSelect(citySelect, translations.selectCity);
            if (shippingZoneSelect) clearSelect(shippingZoneSelect, translations.selectShippingCompany);
            hideShippingInfo();
        ...
        try {
            const data = await fetchJson(`/api/locations/cities?governorate=${governorateId}`);
            if ((data.data || []).length > 0) {
                ...
                if (citySelect) clearSelect(citySelect, translations.selectCity);
                await loadShippingOptions(countrySelect.value, governorateId, null);
            }
        } catch (err) {
    ...
    async function loadShippingOptions(countryId, governorateId, cityId, shippingZoneSelect, currencySymbol, translations, hideShippingInfo, updateShippingDisplay) {
        if (!countryId) {
            hideShippingInfo();
            return;
        }
        ...
        try {
            const data = await fetchJson(`/api/locations/shipping?${params}`);
            if (!data.data || data.data.length === 0) {
                if (shippingZoneSelect) clearSelect(shippingZoneSelect, translations.selectShippingCompany);
                hideShippingInfo();
                return;
            }
            if (shippingZoneSelect) populateSelect(shippingZoneSelect, (data.data || []).map(item => ({
                id: item.zone_id,
                name: `${item.company_name || item.zone_name} - ${currencySymbol}${parseFloat(item.price).toFixed(2)}`
            })));
            if (data.data.length === 1) {
                if (shippingZoneSelect) shippingZoneSelect.value = data.data[0].zone_id;
    ...
    function updateShippingDisplay(shippingOption, shippingCostDisplay, shippingCompanyDisplay, shippingCostInput, shippingDaysInput, shippingDaysDiv, shippingDaysDisplay, shippingInfo, hiddenShippingZoneId, hiddenShippingPrice, hiddenShippingDays, updateOrderSummary) {
        if (!shippingOption) {
            hideShippingInfo();
            return;
        }
        if (shippingCostDisplay) shippingCostDisplay.textContent = `${shippingOption.currencySymbol || '$'}${parseFloat(shippingOption.price).toFixed(2)}`;
        ...
        if (hiddenShippingZoneId) hiddenShippingZoneId.value = shippingOption.zone_id;
        if (hiddenShippingPrice) hiddenShippingPrice.value = shippingOption.price;
        if (hiddenShippingDays) hiddenShippingDays.value = shippingOption.estimated_days || '';
        updateOrderSummary(shippingOption.price);
    }

    function hideShippingInfo(shippingInfo, shippingCostInput, shippingDaysInput, hiddenShippingZoneId, hiddenShippingPrice, hiddenShippingDays, updateOrderSummary) {
    ...
    function updateOrderSummary(shippingCost, currencySymbol, baseTotal) {
        const shippingAmount = $('.shipping-amount');
        if (shippingAmount) shippingAmount.textContent = shippingCost > 0 ? `${currencySymbol}${parseFloat(shippingCost).toFixed(2)}` : '-';
        const newTotal = baseTotal + parseFloat(shippingCost || 0);
        const orderTotal = $('.order-total');
        if (orderTotal) orderTotal.textContent = `${currencySymbol}${newTotal.toFixed(2)}`;
    }
    ...
    function initCheckoutShipping() {
        ...
        const boundHideShippingInfo = () => hideShippingInfo(shippingInfo, shippingCostInput, shippingDaysInput, hiddenShippingZoneId, hiddenShippingPrice, hiddenShippingDays, (cost) => updateOrderSummary(cost, currencySymbol, baseTotal));
        const boundUpdateShippingDisplay = (option) => updateShippingDisplay(option, shippingCostDisplay, shippingCompanyDisplay, shippingCostInput, shippingDaysInput, shippingDaysDiv, shippingDaysDisplay, shippingInfo, hiddenShippingZoneId, hiddenShippingPrice, hiddenShippingDays, (cost) => updateOrderSummary(cost, currencySymbol, baseTotal));
        const boundLoadShippingOptions = (countryId, governorateId, cityId) => loadShippingOptions(countryId, governorateId, cityId, shippingZoneSelect, currencySymbol, translations, boundHideShippingInfo, boundUpdateShippingDisplay);

        countrySelect.addEventListener('change', function () { loadGovernorates(this.value, governorateSelect, citySelect, shippingZoneSelect, translations, boundHideShippingInfo); });
        governorateSelect.addEventListener('change', function () { loadCities(this.value, citySelect, shippingZoneSelect, countrySelect, translations, boundHideShippingInfo, boundLoadShippingOptions); });
        citySelect.addEventListener('change', function () { if (this.value) boundLoadShippingOptions(countrySelect.value, governorateSelect.value, this.value); else boundLoadShippingOptions(countrySelect.value, governorateSelect.value, null); });

        shippingZoneSelect.addEventListener('change', function () { const selectedZoneId = this.value; if (!selectedZoneId) { boundHideShippingInfo(); return; } const params = new URLSearchParams({ country: countrySelect.value }); if (governorateSelect.value) params.append('governorate', governorateSelect.value); if (citySelect.value) params.append('city', citySelect.value); params.append('all', '1'); fetch(`/api/new-shipping/quote?${params}`).then(r => { if (!r.ok || !r.headers.get('content-type')?.includes('application/json')) throw new Error('Invalid response'); return r.json(); }).then(data => { const selectedOption = data.data.find(item => item.zone_id == selectedZoneId); if (selectedOption) boundUpdateShippingDisplay(selectedOption); }).catch(err => console.error('Error updating shipping display:', err)); });

        if (countrySelect.value) loadGovernorates(countrySelect.value, governorateSelect, citySelect, shippingZoneSelect, translations, boundHideShippingInfo);
    }
    ...
    function initAddressSelection() {
        ...
        const boundHideShippingInfo = () => {
            const shippingInfo = $('#shipping-info'); const shippingCostInput = $('#shipping-cost-input'); const shippingDaysInput = $('#shipping-days-input'); const hiddenShippingZoneId = $('#input-shipping-zone-id'); const hiddenShippingPrice = $('#input-shipping-price'); const hiddenShippingDays = $('#shipping-days-input');
            if (shippingInfo) shippingInfo.classList.add('hidden'); if (shippingCostInput) shippingCostInput.value = ''; if (shippingDaysInput) shippingDaysInput.value = ''; if (hiddenShippingZoneId) hiddenShippingZoneId.value = ''; if (hiddenShippingPrice) hiddenShippingPrice.value = ''; if (hiddenShippingDays) hiddenShippingDays.value = '';
        };

        const boundLoadShippingOptions = (countryId, governorateId, cityId) => loadShippingOptions(countryId, governorateId, cityId, $('#shipping-zone-select'), currencySymbol, translations, boundHideShippingInfo, () => {});

        async function selectAddress(radio) { const label = radio.closest('label'); const addrId = label?.dataset?.addrId; const country = label?.dataset?.country; const governorate = label?.dataset?.governorate; const city = label?.dataset?.city; const line1 = label?.dataset?.line1; const phone = label?.dataset?.phone; const name = label?.querySelector('.address-name')?.textContent || ''; if (selectedAddressIdHidden) selectedAddressIdHidden.value = addrId; if (customerNameInput) customerNameInput.value = name; if (customerEmailInput) customerEmailInput.value = doc.querySelector('meta[name="user-email"]')?.content || ''; if (customerPhoneInput) customerPhoneInput.value = phone; if (customerAddressInput) customerAddressInput.value = line1; if (countrySelect) { countrySelect.value = country; await loadGovernorates(country, governorateSelect, citySelect, $('#shipping-zone-select'), translations, boundHideShippingInfo); } if (governorateSelect) { governorateSelect.value = governorate; await loadCities(governorate, citySelect, $('#shipping-zone-select'), countrySelect, translations, boundHideShippingInfo, boundLoadShippingOptions); } if (citySelect) { citySelect.value = city; if (citySelect.value) await boundLoadShippingOptions(countrySelect.value, governorateSelect.value, citySelect.value); else await boundLoadShippingOptions(countrySelect.value, governorateSelect.value, null); } }
