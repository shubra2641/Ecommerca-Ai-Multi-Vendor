...
(function () {
    ...
    {
        doc.querySelectorAll('[data-dropdown].open').forEach(d => { if (d !== except) {
    ...
    function initCompareBadge()
    {
        ...
        window.addEventListener('compare:update', (e) => {
            ...
            badge.textContent = c;
            if (c > 0) {
                badge.classList.add('show'); } else {
                badge.classList.remove('show');
                }
        });
    }
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => { initDropdowns(); initCurrencySwitch(); initCompareBadge(); }); } else {
