            ...
            if (! empty($it['variant'])) {
                ...
                if (is_object($vObj)) {
                    ...
                    if (! $variantLabel && ! empty($vObj->attribute_data)) {
                        $variantLabel = collect($vObj->attribute_data)->map(fn ($v, $k) => ucfirst($k) . ': ' . $v)->values()->join(', ');
                    }
                } else {
                    if (is_string($it['variant']) && strlen($it['variant']) > 0 && ($parsed = @json_decode($it['variant'], true))) {
