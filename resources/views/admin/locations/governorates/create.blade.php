...
@section('content')
@include('admin.partials.page-header', ['title'=>__('Add Governorate')])
<div class="card modern-card">
	<div class="card-header d-flex align-items-center gap-2">
		<h5 class="m-0">{{ __('Add Governorate') }}</h5>
		<div class="ms-auto">
			<a href="{{ route('admin.governorates.index') }}" class="btn btn-outline-secondary btn-sm">{{ __('Back') }}</a>
		</div>
	</div>

	<form class="admin-form" method="POST" action="{{ route('admin.governorates.store') }}">@csrf
		<div class="card-body">
			<div class="mb-3">
