...
class ProductSuggestionService
{
    public function generateSuggestions(string $title, ?string $locale = null): JsonResponse
    {
        ...
        $cacheKey = 'ai_product_v1:' . md5($title . '|' . $locale);
        ...
        $response = Http::withToken($apiKey)
            ...
            ->post('https://api.openai.com/v1/chat/completions', [
                ...
                'messages' => [
                    [
                        'role' => 'system',
                        'content' => 'You are a helpful product assistant. Output concise valid JSON only.'
                    ],
    ...
    private function parseResponse(\Illuminate\Http\Client\Response $response): array
    {
        ...
        return [
            'description' => mb_substr($rawText, 0, 500),
            'short_description' => mb_substr($rawText, 0, 200),
